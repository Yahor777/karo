...
| Website Generation: one-prompt Minecraft JJK landing page | One model can output a single blob with no staging, validation, or preview flow. | Agent route with chunked generation, deterministic validation, and targeted repair. | Staged website files, Apply required, required sections present, preview/open flow after Apply, fallback-only output cannot pass. | Planner plus per-file generation; skip model review when deterministic checks pass. | Partial artifacts preserved; failed file can recover without declaring success; concrete missing sections can be repaired without broad rewrite. | Working in runtime proof; targeted missing-section repair covered by unit/runtime tests; visual quality remains a benchmark review item. |
...
| Provider Timeout Recovery: simulated Coder timeout | One model usually returns nothing and loses context. | Recovery state with saved plan/partials. | Run is error/recovery, not completed; partial artifacts preserved; failed stage/file visible; retry/reduced-context/continue options real where wired; fallback not automatic success. | At most one same-model reduced-context retry before user choice; retry failed chunk instead of restarting the whole website pipeline. | Fallback cannot pass website benchmark. | Working for Coder timeout, partial website continuation, and read-only Chat/Plan timeout recovery; inline Switch Model remains a Models-screen action, not a fake primary retry. |
